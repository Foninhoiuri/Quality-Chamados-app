...
export function TicketComments({ ticketId, onCountChange }: { ticketId: string; onCountChange?: () => void }) {
  const [items, setItems] = useState<TicketComment[] | null>(null)
  const [texto, setTexto] = useState('')
  const [enviando, setEnviando] = useState(false)
  const me = useCurrentUser()
  const canComment = useCan('comentar_chamados')
  ...
  return (
    <div>
      <div className="mb-1.5 text-[11px] font-medium text-slate-400">Andamento</div>
      {items === null ? (
        ...
        <div className="max-h-56 space-y-1.5 overflow-auto pr-1">
          {items.map((c) => (
            <div key={c.id} className="group rounded-lg border border-slate-800 bg-slate-950/40 px-2.5 py-2">
      ...
      {canComment && (
        <div className="mt-2 flex items-end gap-2">
          <textarea
            ...
            placeholder="Escreva o que foi feito… (Ctrl+Enter envia)"
